...
import { showDialog } from "./showdialog.message.js";
    ...
    export async function loadStockIn() {
                try {
                    const response = await fetch("/mant-load-stockin-list");
                    if (!response.ok) {
...
function handlePanelActions() {
    ...
    function editStockIn(params) {
        fetch("/mant-edit-stockin", {
            ...
            .then((data) => {
                ...
                stockinId.textContent = result[0].id;
    ...
    document.querySelector("#add-form").addEventListener("submit", (event) => {
        ...
        const url = "/mant-add-stockin";
        addStockIn(url, formObject);
    });
    
    
    async function addStockIn(url, formObject) {
        try {
            ...
            if (data) {
                ...
                insertIntoStockInList(data.data);
                closeAddDialog();
            }
        } catch (error) {
            console.error(error);
    ...
    async function deleteStockIn(id) {
        try {
            const response = await fetch("/mant-delete-stockin", {
                method: "DELETE",
                headers: { "Content-Type": "application/json" },
