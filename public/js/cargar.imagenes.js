async function cargarImagenes(api, img_contenedor) {
    try {
        ...
        productos.forEach(producto => {
            ...
            input_cantidad.classList.add("cantidad","text-center","w-10","border","border-1","border-gray-500","rounded","bg-[#2a3139]","text-[#d4d8de]");
   ...
   /*  const input = document.getElementById('cantidad_producto');
    ...
    input.addEventListener('input', function() {
        if (this.value < 0) {
            this.value = 0; // Forzar a que el valor sea al menos 0
        }
    });
 */
